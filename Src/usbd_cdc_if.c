...
/* USER CODE BEGIN PV */
/* Private variables ---------------------------------------------------------*/
extern uint32_t pause_for_line_open;
uart_ctx_t * uart_ctx;
USBD_CDC_LineCodingTypeDef *line_coding;

static void set_uart_line_coding(uart_ctx_t * uart_ctx)
{
  /*
...
static int8_t CDC_Control_FS(uint8_t cmd, uint8_t* pbuf, uint16_t length)
{
  ...
  /*******************************************************************************/
    case CDC_SET_LINE_CODING:
      ...
      if (line_coding->bitrate == 0 || line_coding->datatype == 0) break;
      
      //reset/enable all 3 UARTs
      set_uart_line_coding(&ctx.uart1);
      set_uart_line_coding(&ctx.uart2);
      set_uart_line_coding(&ctx.uart3);
      
      pause_for_line_open = 0; //line opened
...
static int8_t CDC_Receive_FS(uint8_t* Buf, uint32_t *Len)
{
  ...
  HAL_UART_Transmit_DMA(ctx.uart1.huart, Buf, *Len);
